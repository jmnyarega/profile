...
async function getTopTracks(fetchWebApi: Function, token: string) {
    return (await fetchWebApi(token, 'v1/me/top/tracks?time_range=long_term&limit=50', 'GET')).items;
}
    ...
    useEffect(() => {
        async function getTracks() {
            ...
            const tracks = topTracks.map(track => ({
                name: track.name, external_urls: track.external_urls
            }));
            setTracks(tracks);
        }
